...
import { View,Text, Image, StyleSheet, ActivityIndicator, NativeModules, Animated,TouchableOpacity, Keyboard, FlatList, Modal,ScrollView} from 'react-native'
...
export default function LinkScreen({navigation,route}){
    ...
    const { token, deviceData, handleData,setBgColor,bData, setDeviceData,reloadStaffList,getDeviceData } = useContext(AccountContext)
    ...
    useEffect(async() => {
        ...
        console.log(deviceData)
            await axios.get(constants.plantApi,{
                headers: {
                    Authorization: authToken,
                    "Fcm-Imei":deviceData.imei
                    },
    ...
    const renderItem = ({ item, index}) => {
        ...
        return(
            <TouchableOpacity key={item.id+'_'+item.name_eng} style={{marginTop:'3%',flexDirection:'row',paddingVertical:'5%'}} onPress={()=>pressHandler(index)}>
            <View style={{flex:0.2}}>
    ...
    const renderImageItem = ({ item, index}) => {
        ...
        return(
            <TouchableOpacity key={'image_'+index} style={{aspectRatio:1}} onPress={() => pressHandler(index)} >
                ...
                <Image style={{width:70*constants.HeightRatio,height:70*constants.HeightRatio}} resizeMode='cover' source={{isStatic:true,uri:item.img?item.img:item}} key={'image_'+item.id} />
            </TouchableOpacity>
        )
            ...
            <View style={{flex:1}}>
                ...
                <Animated.View style={[{borderTopLeftRadius:25,borderTopRightRadius:25,backgroundColor:'white',justifyContent:'center',position:'absolute',width:'100%'},{height:interpolatedHeight,bottom:moveBot}]}>
                    <View style={{alignSelf:'flex-end',marginTop:'7.5%',marginRight:'10%'}}>
                        {deviceData.plant_id !== "" ?  
                            ...
                            :
                            <TouchableOpacity onPress={()=>setLogin(true)}>
                                <Text style={{color:'#73B2B8',fontFamily:'M-SB',fontSize:14,letterSpacing:1}}>{i18n.t('Add')}{i18n.t('Plant')}</Text>
                            </TouchableOpacity>}
                                ...
                                <View style={{width:'80%'}}>
                                    ...
                                    <>
                                        ...
                                        <View style={{flexDirection:'row',marginTop:'3%'}}>
                                                ...
                                                <View>
                                                    <Text style={{fontFamily:'M-SB',color:'#73B2B8',fontSize:18}}>{i18n.t('Plant')}{i18n.t('Image')}</Text>
                                                     <FlatList  style={{marginTop:'3%'}} horizontal={true} data={deviceData.image}  renderItem={renderImageItem} keyExtractor={item => item.name} />
                                    ...
                                    {plant!==''?
                                        <View>
                                            <View style={{marginTop:'4%'}}>
                                                <Text style={{color:'rgba(0,0,0,0.6)',fontFamily:'M-M',fontSize:12}}>{i18n.t("Name")}</Text>
                                                <Text style={{color:'rgba(0,0,0,0.87)',fontFamily:'M-SB',fontSize:16}}>{plant.label}</Text>
                                            </View>
                                            <View style={{marginTop:'4%'}}>
                                                <Text style={{color:'rgba(0,0,0,0.6)',fontFamily:'M-M',fontSize:12}}>{i18n.t("Location")}</Text>
                                                <Text style={{color:'rgba(0,0,0,0.87)',fontFamily:'M-SB',fontSize:16}}>{plant.remark}</Text>
                                            </View>
                                            {plant.driver.length>0?
                                                <View style={{marginTop:'4%'}}>
                                                    <Text style={{color:'rgba(0,0,0,0.6)',fontFamily:'M-M',fontSize:12}}>{i18n.t("Driver")}</Text>
                                                    <FlatList horizontal={true} data={plant.driver}  renderItem={renderDriverItem} keyExtractor={item => item.full_name} />
                                            ...
                                            <View style={{flexDirection:'column',marginTop:'3%'}}>
                                                ...
                                                <View>
                                                    ...
                                                    <FlatList horizontal={true} style={{marginTop:'3%'}} data={plant.img}  renderItem={renderImageItem} keyExtractor={item => item.name} />
                    ...
                    <View style={{height:100,justifyContent:'center'}}>
                        ...
                        <TouchableOpacity  onPress={()=>handlerSubmit()}  style={{width:'80%',alignSelf:'center',backgroundColor:'#73B2B8',borderWidth:0,elevation:2,marginBottom:'10%',paddingVertical:'3%',alignItems:'center',borderRadius:5,marginTop:'5%'}}> 
                            {submit?<ActivityIndicator size="small" color="white" />:<Text style={[styles.btnContent]}>{i18n.t("LINK")}</Text>}
                        </TouchableOpacity>
                ...
                {expanded?
                    <>
                        {deviceData.image.length !==0 ?
                            <ImageView
                                images={deviceData.image.map(item=> {return {uri:item}})}
                    ...
                    {plant!=="" && plant.img.length!==0 ?
                        <ImageView
                            images={plant.img.map(item=> {return {uri:item.img}})}
                            imageIndex={selectedImnage!==-1?selectedImnage:0}
                            visible={isVisible}
                            onRequestClose={() => setIsVisible(false)}
                        />                  
