...
export function HomeDrawer(props) {
  ...
  const renderItem = ({item, index}) => {
    ...
    return (
      <TouchableOpacity style={{height: constants.windowHeight / 4, marginVertical: '3%'}} key={item.id}
  ...
  const ProjBox = () => <Modal transparent={true}
    ...
    <View style={styles.centeredView}>
      <View style={styles.modalView}>
        ...
        <FlatList style={{marginBottom: '3%'}} data={projectList} renderItem={renderItem}
                  keyExtractor={item => item.id}/>
        <TouchableOpacity onPress={() => setModalVisible(!modalVisible)} style={[styles.submitBtn, {
